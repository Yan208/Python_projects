...
ml_testapp = globals()
ml_testapp = ObjectProperty(None)
...
class MainLayout(Screen):
  ...
  def receive_self_testapp_in_mainlay(self, self_testapp):
      global ml_testapp
      ml_testapp = self_testapp
  ...
  def back_to_file(self):
    ml_testapp.sm.get_screen('menu_file').back_from_ml = True
    self.text_area.focus = False
    self.save_file()
    fm.self_file.build_file_menu()
    ml_testapp.sm.current = 'menu_file'
  ...
  def save_file(self):
    ...
    if self.file_name == ".txt":
      ml_testapp.sm.get_screen('menu_file').text_for_label = "Не сохранил"
      return
    print("имя файла:", self.file_name)
    try:
      with open(self.file_name, 'w') as file:
        file.write(self.text_area.text)
      ml_testapp.sm.get_screen('menu_file').text_for_label = f'Сохранил в {self.file_name}'
      print('сохранил и сменил лэйбл')
    except:
      print("ОШИБКА ЗАПИСИ")
      ml_testapp.sm.get_screen('menu_file').text_for_label = f'ОШИБКА: {self.file_name[:90]}'
    finally:
