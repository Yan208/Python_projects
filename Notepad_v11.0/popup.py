...
class PopupFile(BoxLayout):
  ...
  def __init__(self, **kwargs):
    ...
    global file_fun, self_popup, s_man
    #, popup_testapp
    self_popup = self
  ...
  def popup_file(self, file_list_building):
    ...
    if not self.is_popup_in_float():
      ...
      print('выполнено add_widget из popup_file')
    ...
    for i in range(len(self.file.files_txt)):
      ...
      popup_file_menu.button_list_menu[i].unbind(on_single_press = popup_file_menu.choice_file_main)
    return
  ...
  def popup_open(self):
    ...
    self.is_popup = False
    if file_list_obj is not None:
      file_list_building = file_list_obj
      print("В переменной есть значение")
    else:
  ...
  def popup_delete(self):
    ...
    self.app.sm.get_screen('menu_file').build_file_menu()
    if self.is_popup_in_float():
      self.app.sm.get_screen('menu_file').float.remove_widget(self.box)
    return
